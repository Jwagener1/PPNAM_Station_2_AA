...
class MqttTopicsTest {
    ...
    fun `request topic lowercases and removes spaces from station name`() {
        assertEquals("PPNAM/station2/request", MqttTopics.request("Station 2"))
    }

    @Test
    fun `request topic handles station 3`() {
        assertEquals("PPNAM/station3/request", MqttTopics.request("Station 3"))
    }

    @Test
    fun `response topic includes device id`() {
        assertEquals("PPNAM/station2/response/abc123", MqttTopics.response("Station 2", "abc123"))
    }
    ...
    @Test
    fun `hopperStatus topic is under the PPNAM namespace`() {
        assertEquals("PPNAM/station2/hopper/status", MqttTopics.hopperStatus("Station 2"))
    }
