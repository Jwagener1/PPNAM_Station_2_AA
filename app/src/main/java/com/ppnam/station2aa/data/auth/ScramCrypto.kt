...
object ScramCrypto {
    ...
    /**
     * SASLprep-lite: Unicode Normalization Form KC, per the contract's step 1 ("normalize the
     * password with Unicode Form KC and encode UTF-8").
     *
     * Full RFC 4013 SASLprep additionally maps non-ASCII spaces to U+0020 and strips soft
     * hyphens/zero-width joiners. NFKC already folds the great majority of that; the contract asks
     * for KC specifically and Station 2 is the other side of this computation, so matching what it
     * documents is what makes the proof verify.
     */
     ...
     * to satisfy the contract's "validate the returned `serverSignature` before trusting the
     * session" — so returning them together keeps a caller from being able to skip that check by
    ...
    // java.util.Base64, not android.util.Base64. Two reasons, both load-bearing:
    //  - android.util.Base64.DEFAULT line-wraps, which would corrupt every value on the wire;
    //  - this module sets `isReturnDefaultValues = true`, so the stub android.jar would return
    //    NULL from android.util.Base64 in unit tests instead of throwing. Every SCRAM test would
    //    then exercise a null proof and still look like it passed.
    // java.util.Base64 is API 26 and this module's minSdk is 26, so it is real on both sides.
